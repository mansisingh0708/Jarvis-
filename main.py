...
#         messages=[
#         {"role": "system", "content": "You are Jarvis Helpful AI assistant Give ans in 3-4lines"},
...
def speak(text):
    try:
        ...
        pass
def get_news_mediastack():
    api_key = 'e235f2c6e1f3838b3e98eb8a1d0ed4e5'
    url = f'http://api.mediastack.com/v1/news?access_key={api_key}&countries=in&limit=5'  # reduced to 5
    try:
...
def ask_local_llm(prompt):
    try:
        is_detailed = any(word in prompt.lower() for word in ["detail", "detailed", "long", "elaborate"])
        
        if is_detailed:
            system_msg = "You are Jarvis. Provide a detailed, 200-word explanation. Be factually accurate and thorough."
            token_limit = 500
        else:
            system_msg = (
                "You are Jarvis. You provide EXTREMELY brief, 3-sentence factual answers. Always end the sentence when a period is reached. Dont halt before that "
            )
            token_limit = 100 
        ...
        return "I am sorry, I encountered an internal processing error."

def listen_for_command(r, timeout=6, phrase_limit=8):
    """Helper to listen and return command text, or None on failure."""
    try:
        with sr.Microphone() as source:
...
def processcommand(command):
    ...
    elif "open leetcode" in command or "open leet code" in command:
        webbrowser.open("https://leetcode.com/problemset/javascript/")
        speak("Opening LeetCode")
    elif "iiit pune" in command or "triple i t pune" in command:
        speak("Opening IIIT Pune website")
        webbrowser.open("https://www.iiitp.ac.in/")
    elif command.startswith("play"):
        song = command.replace("play", "").strip()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        try:
            ...
            if "jarvis" in word:
                speak("Hi, I am Jarvis. How can I help you? Say exit to stop.")

                while True:
                    command = listen_for_command(r)

                    if command is None:
                        speak("I'm still here. What do you need?")
                        continue
